        ...
        while k <= n_sim:
            ...
            try:
                ...
                for i, well_idx in enumerate(chosen_wells_idxs):
                    tasks.append((
                        well_idx,
                        self.wells[well_idx].wp,
                        self.wells[well_idx].bc,
                        well_deltas[i],
                        self.x_guesses[well_idx]
                    ))
                
                # simulate the wells in parallel
                with mp.Pool(processes=min(mp.cpu_count(), n_sim_wells, 4)) as pool:
                    results = pool.starmap(simulate_perturbations, tasks)
                
                for well_idx, x_pos, x_neg in results:
                    well = self.wells[well_idx]
                    self.x_guesses[well_idx].append(x_pos)
                    self.x_guesses[well_idx].append(x_neg)

                    well.bc.u = well_deltas[well_idx][0][0]
                    well.bc.w_lg = well_deltas[well_idx][0][1]
                    if x_pos is None:
                        dp_pos, tmp = self._choked_flow(well, sample_type='Perturbation')
                    else:
                        dp_pos = create_data_point(well=well, sim=simulators[well_idx], x=x_pos, decision_type='Perturbation')

                    well.bc.u = well_deltas[well_idx][1][0]
                    well.bc.w_lg = well_deltas[well_idx][1][1]
                    if x_neg is None:
                        dp_neg, tmp = self._choked_flow(well, sample_type='Perturbation')
                    else:
                        dp_neg = create_data_point(well=well, sim=simulators[well_idx], x=x_neg, decision_type='Perturbation')

                    pos_well_data[well_idx] = dp_pos
                    neg_well_data[well_idx] = dp_neg
            ...
            #         well_data[well_idx] = pd.concat([well_data[well_idx], dp_pos, dp_neg], ignore_index=True)
                ...
                # simulate the wells in parallel
                with mp.Pool(processes=min(mp.cpu_count(), n_sim_wells, 4)) as pool:
                    results = pool.starmap(simulate_optmizing, tasks)
                
                for well_idx, x in results:
                    well = self.wells[well_idx]
                    self.x_guesses[well_idx].append(x)

                    well.bc.u = opt_theta[well_idx][0]
                    well.bc.w_lg = opt_theta[well_idx][1]
                    if x is None:
                        dp, tmp = self._choked_flow(well, sample_type='Optimizing')
                    else:
                        dp = create_data_point(well=well, sim=simulators[well_idx], x=x, decision_type='Optimizing')
...
def load_well_configs():
    ...
    """
    dataset_filename = 'data/testing/SPSA-sol_configs/mixed_prod_choke50%.csv'
    dataset = pd.read_csv(dataset_filename)
    return dataset

def simulate_perturbations(well_idx:int, wp: WellProperties, bc:BoundaryConditions, deltas: list, x_guesses: list):
    """
    Simulate positive and negative perturbations for a single well.
    Returns: (well_idx, dp_pos, dp_neg, x_pos, x_neg)
    """
    # deep copy so we don't stomp on shared state
    sim_copy = sim = SSDFSimulator(wp, bc)

    # Unpack the two perturbed settings:
    (u_pos, gl_pos), (u_neg, gl_neg) = deltas

    # Positive run
    sim_copy.bc.u, sim_copy.bc.w_lg = u_pos, gl_pos
    try:
        x_pos = single_SPSA_sim(
            key=well_idx, sim=sim_copy, x_guesses=x_guesses
        )
    except SimError:
        # retry once or handle failure
        x_pos = single_SPSA_sim(
            key=well_idx, sim=sim_copy, x_guesses=x_guesses
        )

    # Negative run
    sim_copy.bc.u, sim_copy.bc.w_lg = u_neg, gl_neg
    try:
        x_neg = single_SPSA_sim(
            key=well_idx, sim=sim_copy, x_guesses=x_guesses
        )
    except SimError:
        x_neg = single_SPSA_sim(
    ...
    return well_idx, x_pos, x_neg

def simulate_optmizing(well_idx:int, wp: WellProperties, bc:BoundaryConditions, opt_theta: list, x_guesses: list):
    """
    Simulate the well with the optimized decision variables.
    Returns: (well_idx, dp, x)
    """
    # deep copy so we don't stomp on shared state
    sim_copy = SSDFSimulator(wp, bc)

    # Set the decision variables
    sim_copy.bc.u, sim_copy.bc.w_lg = opt_theta
    ...
    try:
        ...
        raise SimError(f"Simulation failed for well {well_idx} with optimized decision variables.")

def single_SPSA_sim(key: int, sim: SSDFSimulator, x_guesses: list):
    """
    Iterates through one simulation of a well.
    :param well: Well object
    :param sim: Simulator object
...
if __name__ == '__main__':
    dataset_version = 'testing/testing-spsa-25'
    ...
    for i in range(10):
        ...
        opt = SPSAOptimization(wells=wells, sigma=4.)
        try:
            opt.SPSA(n_sim=50, dataset_version=f"{dataset_version}/run{i}")
        except SimError as e:
            print(f"Simulation failed: {e}")
            continue
