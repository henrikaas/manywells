...
class SPSAOptimization:
    ...
    def __init__(self, wells: dict[Well], 
                constr: OptimizationProblemProperties = OptimizationProblemProperties(),
                a: float = 0.1, b: float = 0.5, c: float = 0.1,
                A: int = 50,
                alpha: float = 0.301, beta: float = 0.602, gamma: float = 0,
                sigma: float = 0.0,
                config: dict = None):
        ...
        # SPSA secific hyperparametres
        self.a = a # Learning rate controller
        self.b = b # Dual rate controller
        self.c = c # Perturbation magnitude controller
        self.A = A # Stabilizer
        self.alpha = alpha # Decay rate for learning rate
        self.beta = beta # Decay rate for dual rate
        self.gamma = gamma # Decay rate for perturbation size
    ...
    def _single_SPSA_sim(self, key: int, well: Well, sim: SSDFSimulator, sample_type: str = 'Optimizing'):
        """
    ...
    def _choked_flow(self, well: Well, sample_type:str):
        """
    ...
    def spsa(self, well: int, n_sim: int, dataset_version: str = None):
        """
        Runs an spsa algorithm on single well, trying to find the optimal produciton solution.
        Loosely based on scripts.data_generation.open_loop_stationary.generate_well_data

        :param well: index of well in self.wells
        :param n_sim: Number of data points to simulate
        """
        well = self.wells[well]

        well_data = create_sim_results_df()
        sim = SSDFSimulator(well.wp, well.bc)

        n_attempts = 0 # Number of simulation attempts
        max_attempts = 3 * 5 * n_sim # Max number of attempts
        n_failed_sims = 0 # Number of failed simulations
        max_fails = 100 # Maximum number of allowed fails

        # Storage of decision variables in each run
        theta = [[well.bc.u, well.bc.w_lg]]

        k = 1 # Iteration operator
        while k <= n_sim:
            if n_attempts >= max_attempts:
                raise SimError('Discarding well: maximum number of simulation attempts reached')
            n_attempts += 1

            if n_failed_sims >= max_fails:
                raise SimError('Discarding well: too many simulation failures')
            
            # Back up current well
            backup_well = well.copy()
            
            # Sample new well conditions
            new_well = well.sample_new_conditions(sample_u=False, sample_w_lg=False)
            well = new_well
            sim.wp = well.wp
            sim.bc = well.bc

            # Draw perturbation directions
            cur_u = theta[-1][0]
            cur_gl = theta[-1][1]
            ak = self.a / ((k + self.A) ** self.alpha) # Learning rate
            ck = self.c / (k ** self.gamma) # Perturbation size

            guesses, directions = self._sample_variables(current_u=cur_u, current_gl=cur_gl, ck=ck)

            # Evaluate the objective at perturbation points
            # Point #1
            try:
                u = guesses[0][0]
                w_gl = guesses[0][1]
                # print(f"Point 1: u = {u}, gl = {w_gl}")

                sim.bc.u = u
                sim.bc.w_lg = w_gl

                x = sim.simulate()
                x1_dp = create_data_point(well=well, sim=sim, x=x, decision_type='Perturbation')
                y1 = x1_dp['WOIL'].values[0]
            ...
            try:
                u = guesses[1][0]
                w_gl = guesses[1][1]
                # print(f"Point 2: u = {u}, gl = {w_gl}")

                sim.bc.u = u
                sim.bc.w_lg = w_gl

                x = sim.simulate()
                x2_dp = create_data_point(well=well, sim=sim, x=x, decision_type='Perturbation')
                y2 = x2_dp['WOIL'].values[0]
            ...
            # Calculate direction
            grad_u = (y1 - y2) / (2 * ck * directions[0])
            grad_gl = (y1 - y2) / (2 * ck * directions[1]) if well.has_gas_lift else 0

            opt_theta = [cur_u + grad_u * ak, cur_gl + grad_gl * ak * self.constr.gl_max]
            opt_theta = np.clip(opt_theta, [self.constr.u_min, self.constr.gl_min], [self.constr.u_max, self.constr.gl_max])

            # Sample new well conditions
            new_well = well.sample_new_conditions()
            well = new_well

            well.bc.u = opt_theta[0]
            well.bc.w_lg = opt_theta[1]
            sim.wp = well.wp
            sim.bc = well.bc

            # Simulate chosen optimizing direction
            try:
                x = sim.simulate()

                opt_dp = create_data_point(well=well, sim=sim, x=x, decision_type='Optimizing')
            ...
            print(f"SPSA specific values: ak: {ak}, ck: {ck}")
            print(f"Production point #1: {y1}, Production point #2: {y2}")
            print(f"Calculated gradients: u = {grad_u}, gl = {grad_gl}")
            print(f"Final decision variables: u = {opt_theta[0]}, gl = {opt_theta[1]}")
            print(f"Optimizing production: {opt_dp['WOIL']}")

            well_data = pd.concat([well_data, x1_dp, x2_dp, opt_dp], ignore_index=True)
            theta = theta + guesses
            theta = theta + [opt_theta]
            k += 1
        
        # SimErrors:
        if len(well_data) / 3 != n_sim:
        ...
        print(f"Decisions: {theta}")

    def run_multiple_SPSA(self, n_sim: int, dataset_version: str = None):
        """Deprecated"""

        n_sim_wells = self.constr.max_moves // 2 # Number of wells to simulate
        well_idxs = np.arange(0, len(self.wells))
        ...
        sampled_directions = [[0,0]]*n_sim_wells # Placeholder for sample of dec. vars in each well
        ...
        while k < n_sim:
            ...
            chosen_wells_idxs = np.sort(np.random.choice(well_idxs, n_sim_wells, replace=False)) # Randomly choose wells to perturbate
            chosen_wells = [well for i, well in enumerate(self.wells.values()) if i in chosen_wells_idxs]
            cur_state = np.array([[w.bc.u, w.bc.w_lg] for w in chosen_wells]) # Current state of the decision vars in the chosen wells

            # Calculate SPSA parametres
            ak = self.a / ((k + self.A) ** self.alpha)
            ck = self.c / (k ** self.gamma)

            # Draw directions for the wells to perturbate in
            # Each pair of wells (i, i+1) perturbate in opposite direction for each iteration
            bern_directions = [self._draw_directions() for _ in range(n_sim_wells // 2)] # Randomly drawn bernoulli directions for each pair
            for i, d in enumerate(bern_directions):
                # Well 1
                well_directions[2*i] = d[0], d[1] if self.wells[chosen_wells_idxs[2*i]].has_gas_lift else 0
                sample1, ø = self._sample_variables(current_u=self.wells[chosen_wells_idxs[2*i]].bc.u, 
                                                    current_gl=self.wells[chosen_wells_idxs[2*i]].bc.w_lg,
                                                    ck=ck, directions=well_directions[2*i])
                sampled_directions[2*i] = sample1

                # Well 2
                well_directions[2*i+1] = -d[0], -d[1] if self.wells[chosen_wells_idxs[2*i+1]].has_gas_lift else 0
                sample2, ø = self._sample_variables(current_u=self.wells[chosen_wells_idxs[2*i+1]].bc.u,
                                                    current_gl=self.wells[chosen_wells_idxs[2*i+1]].bc.w_lg,
                                                    ck=ck, directions=well_directions[2*i+1])
                sampled_directions[2*i+1] = sample2

            pos_Ys = [None] * n_sim_wells
            oil_pos = np.zeros(n_sim_wells)
            neg_Ys = [None] * n_sim_wells
            oil_neg = np.zeros(n_sim_wells)

            # Simulate the wells that are perturbed
            for i, well in enumerate(chosen_wells):
                # Positive direction
                # Change decision variables
                well.bc.u = sampled_directions[i][0][0]
                well.bc.w_lg = sampled_directions[i][0][1]
                try:
                    # Simulate positive direction.
                    dp_pos = self._single_SPSA_sim(key=i, well=well, sim=simulators[chosen_wells_idxs[i]], sample_type='Perturbation')

                except SimError as e:
                    # If the simulation fails, we back up the well, sample new conditions and try again
                    print(f"Simulation failed for well {i}, trying again...")
                    # n_failed_sims += 1

                    # Reset well
                    well, simulators[chosen_wells_idxs[i]] = self._handle_simulation_failure(well_key=chosen_wells_idxs[i], sim=simulators[chosen_wells_idxs[i]],
                                                                                             u=sampled_directions[i][0][0], gl=sampled_directions[i][0][1])
                ...
                # Negative direction
                # Change decision variables
                well.bc.u = sampled_directions[i][1][0]
                well.bc.w_lg = sampled_directions[i][1][1]
                try:
                    # Simulate negative direction.
                    dp_neg = self._single_SPSA_sim(key=i, well=well, sim=simulators[chosen_wells_idxs[i]], sample_type='Perturbation')
                    ...
                    # Reset well
                    well, simulators[chosen_wells_idxs[i]] = self._handle_simulation_failure(well_key=chosen_wells_idxs[i], sim=simulators[chosen_wells_idxs[i]],
                                                                                             u=sampled_directions[i][1][0], gl=sampled_directions[i][1][1])
                ...
                # Store results
                pos_Ys[i] = dp_pos
                oil_pos[i] = dp_pos['WOIL'].values[0]
                neg_Ys[i] = dp_neg
                oil_neg[i] = dp_neg['WOIL'].values[0]
                well_data[chosen_wells_idxs[i]] = pd.concat([well_data[chosen_wells_idxs[i]], dp_pos, dp_neg], ignore_index=True) # Store in data for well
            ...
            for i, well in enumerate(self.wells.values()):
                if i not in chosen_wells_idxs:
                    try:
                        # Simulate the well
                        dp = self._single_SPSA_sim(key=i, well=well, sim=simulators[i], sample_type='Stationary')
                    except SimError as e:
                        print(f"Simulation failed for well {i}, trying again...")
                        # n_failed_sims += 1
            ...
            # Calculate the state of the system ito production & constraints
            self.calculate_state(well_data=well_data)
            
            # Calculate gradients
            oil_grads = (oil_pos - oil_neg).reshape(-1, 1) / (2 * ck * well_directions)
            for i in range(n_sim_wells): # Set w_gl in wells without gas lift to 0
                if not self.wells[chosen_wells_idxs[i]].has_gas_lift:
                    oil_grads[i][1] = 0
                else:
                    oil_grads[i][1] *= self.constr.gl_max
            
            opt_theta = cur_state + oil_grads * ak
            opt_theta = np.clip(opt_theta, [self.constr.u_min, self.constr.gl_min], [self.constr.u_max, self.constr.gl_max])
            ...
            for i, well in enumerate(chosen_wells):
                ...
                try:
                    # Simulate
                    dp = self._single_SPSA_sim(key=i, well=well, sim=simulators[chosen_wells_idxs[i]], sample_type='Optimizing')
                ...
                # Store results
                well_data[chosen_wells_idxs[i]] = pd.concat([well_data[chosen_wells_idxs[i]], dp], ignore_index=True)
            ...
            print("--------------------------------------------")
            print(f"Simulation #{k} successful.")
            print(f"SPSA specific parametres: ak: {ak}, ck: {ck}")
            print("--------------------------------------------")

            self.backup_wells = self.wells.copy() # Back up of wells for next iteration
            k += 1
        
        if dataset_version is not None:
            save_well_config_and_data(config=self.wells.values(), data=pd.concat(well_data, ignore_index=True), dataset_version=dataset_version)
